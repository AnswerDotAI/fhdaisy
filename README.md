...
`fhdaisy` provides Python components for using [DaisyUI](https://daisyui.com/) in [FastHTML](https://www.fastht.ml/) applications. For example, `Btn('Click me', cls='-primary')` creates `<button class="btn btn-primary">Click me</button>`. Component names follow DaisyUI’s CSS classes: `card` becomes `Card`, and `alert` becomes `Alert`. Modifiers can omit the component prefix, as in `-primary` for `btn-primary`.
...
Use [`mk_previewer()`](https://answerdotai.github.io/fhdaisy/core.html#mk_previewer) to preview components in Jupyter, Solveit, and similar environments:
...
`fhdaisy.xtras` provides [`mk_accordion_item`](https://answerdotai.github.io/fhdaisy/xtras.html#mk_accordion_item) to construct those parts:
...
[`mk_accordion`](https://answerdotai.github.io/fhdaisy/xtras.html#mk_accordion) builds an accordion from a sequence of items. All xtras helpers use the `mk_` prefix to distinguish them from components that map directly to DaisyUI classes:
...
Try components interactively in Jupyter or Solveit. Create a previewer with [`mk_previewer()`](https://answerdotai.github.io/fhdaisy/core.html#mk_previewer) and compare different combinations and modifiers. Start with `Btn` or `Alert` and modifiers such as `-primary`, `-outline`, and `-lg`. Then try multi-part components such as cards and modals.
...
See the [DaisyUI component documentation](https://daisyui.com/components/) for all components and modifiers. See the [FastHTML documentation](https://www.fastht.ml/) for building complete applications.

Contributions of useful `mk_` helpers are welcome in the [fhdaisy repository](https://github.com/AnswerDotAI/fhdaisy). Use that prefix for helpers that construct repeated component structures.
